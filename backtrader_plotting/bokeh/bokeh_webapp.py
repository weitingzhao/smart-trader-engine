...
class BokehWebapp:
    def __init__(self, title, html_template, scheme, model_factory_fnc, on_session_destroyed=None, port=8090):
        self._title = title
    ...
    def _run_server(fnc_make_document, iplot=True, notebook_url="localhost:8889", port=80, ioloop=None):
        ...
        if iplot and 'ipykernel' in sys.modules:
            ...
            print(f"Browser is launching at: http://10.0.0.90:{port}")
            threading.Timer(2, lambda: webbrowser.open(f'http://10.0.0.90:{port}')).start()
